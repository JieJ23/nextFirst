...
export default async function ProvideData() {
    ...
    const res = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=contentDetails&id=${channelString.join(",")}&key=${YOUR_API_KEY}`);
    ...
    const resProfile = await fetch(`https://www.googleapis.com/youtube/v3/channels?part=snippet&id=${channelString.join(",")}&key=${YOUR_API_KEY}`)
    ...
    async function getVidData(id) {
        const videosRes = await fetch(`https://www.googleapis.com/youtube/v3/playlistItems?part=snippet&maxResults=4&playlistId=${id}&key=${YOUR_API_KEY}`);
